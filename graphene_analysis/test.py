...
def get_path_to_file(directory_path, file_suffix,file_praefix=None):
    """
    Return the path to file given directory, praefix and suffix.

    Arguments:
        directory_path: The path to the directory containing the trajectory.
        file_suffix: The general suffix or type of the file.
        file_praefix: The general name of the file.

    Returns:
        path_to_file: The path to the requested file.
    """

    files_in_path = glob.glob(os.path.join(directory_path, f"*.{file_suffix}"))
    ...
    if file_praefix:
        return [
            file for file in files_in_path if file_praefix + f".{file_suffix}" in file
        ][0]
...
def get_ase_atoms_object(pdb_file_path):
    ...
    try: 
        ...
        file_praefix = (pdb_file_path.split('/'))[-1].split('.')[0] 
        print(f"Looking for lammpstrj files in {directory_path}")

        # Look for lammpstrj files in same directory
        lammpstrj_file = get_path_to_file(directory_path,file_praefix,"lammpstrj")
